...
- For external file drags from a sandboxed Clover surface, also start the relevant security-scoped bookmark before writing `public.file-url` pasteboard data and keep it open until the drag session ends. If this is missed, target apps can reject the drop with logs such as `Sandbox extension creation failed`, `Sandbox extension data required immediately for flavor public.file-url`, or `No sandbox extension entry in cache`.
...
- Keep basic pane layouts free: single, two vertical, and two horizontal. Four-grid plus advanced three-pane layouts such as left-one-right-two, left-two-right-one, top-one-bottom-two, and top-two-bottom-one are Pro-only and should trigger the upgrade window when selected without an active Pro entitlement.
- Keep Release monetization UI purchase-focused: show Upgrade/Purchase entry points before purchase, hide Upgrade to Clover Pro once the lifetime product is active, and keep a distinct Restore Purchases action visible in release wherever purchase options are offered. Keep Manage Subscription behind Debug-only controls unless the user explicitly asks to ship it. Prefer the shared OmniMAC-derived paywall interaction from `macos-app-common/references/purchases.md` for future purchase UI changes.
...
- For external drag compatibility, prefer file-only pasteboard types such as `.fileURL` and `NSFilenamesPboardType`; do not add `.string` or generic `.URL` just to make text inputs respond. `.string` makes Codex/browser inputs insert a path, and `.URL` can trigger `about:blank#blocked` navigation instead of file attachment.
- Pane and stash-shelf drag sources must hold security-scoped access for dragged file URLs before pasteboard writing and release it from the drag session ended callback.
...
- Stash shelf drags are sandbox-sensitive too: resolved stashed-file bookmarks must stay security-scoped for the duration of the drag session, otherwise external targets may fail to import the file despite seeing `public.file-url`.
- When a stash shelf file drag ends with a non-empty drag operation and the release point is outside the stash shelf window, treat the transfer as successful and clear the stash shelf. Cancelled/rejected drags, or drags released back on the shelf, must leave the stash unchanged.
- While a stash shelf file drag is in progress, hide the shelf's in-place file thumbnails and count badge by setting their opacity to 0; restore opacity to 1 when the drag ends before deciding whether to clear the stash.
