...
- During file drags, hovering a browsable directory should visibly select that directory as the drop target and auto-expand it after a short delay, including drags that started in the same pane. Drop target calculation should use the current pointer location, not stale cell row tags or AppKit's proposed insertion row.
- When dragging over a file or folder row, collapse expanded sibling directories at the same list depth so unrelated same-level directories do not remain expanded while the drag target changes.
...
- If a Pro stash shelf source drag ends with a non-empty operation such as `.copy` and the release point is outside the shelf window, clear the stash; keep it unchanged when the drag is cancelled, rejected, or released back on the shelf.
- During a Pro stash shelf source drag, set the in-place shelf thumbnail stack and count badge opacity to 0 so only the drag image represents the files; restore them to 1 when the drag ends, before any successful-transfer clear.
